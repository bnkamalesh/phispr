...
type Config struct {
	...
	Rooms struct {
		Capacity                 uint
		MemberCapacity           uint
		LiveViewerBroadcastDelay time.Duration
...
func Load(path string) *Config {
	...
	cfg.Rooms.LiveViewerBroadcastDelay = k.Duration("rooms.live_viewer_broadcast_delay")
	cfg.Rooms.BackupFilePath = k.String("rooms.backup_filepath")
	cfg.Rooms.IdleRoomExpiry = k.Duration("rooms.idle_room_expiry")
